  ...
  public function taobao_collect($date,$p){
		...
		if(preg_match_all(L($pre."listitem"), $content, $matchitem)) {
					...
					for($i=0;$i<count($matchitem[0]);$i++){
						...
						//add
						if (preg_match('%http://item\.taobao\.com(?:.*?)#detail%sim', $item,$matches)) {
							$itemurl=$matches[0];
								
							$ftxia_https_item = new ftxia_https();
							...
							if (preg_match("%\d+.00\s+-\s+\d+.00%", $result_tmp)) {
						...
						if (preg_match('%http://detail\.tmall\.com/item\.htm(?:.*?)\d+%sim', $item,$matches)) {
							$itemurl=$matches[0];
							...
							$result_tmp = Newiconv("GBK", "UTF-8", $result_tmp);
							$original_price=get_word($result_tmp,L($pre.'tmall_price_start'),L($pre.'tmall_price_end'));
						}
					   ...
					   $times	=	(int)(time()+72*86400);
						...
						if($original_price==$price){
							$coupon_rate=$zekou*1000;
						}else{
							$coupon_rate=intval($price)/intval($original_price)*100;
							$coupon_rate=round($coupon_rate)*100;
						}
						...
						$itemarray['original_price']=$original_price;
